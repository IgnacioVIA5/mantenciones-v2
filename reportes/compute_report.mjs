...
/* ═══════════════════════ REPORTE SEMANAL — lógica agregada ═══════════════════════ */
import fs from "fs";
...
function calcularEstado(e) {
  ...
  return {
    ...
    restPrevKm, restGenKm, useKmForPrev, useKmForGen,
    estPrev: ep, estGen: eg, salud: worst,
  };
}

function ultimaFechaCorrectiva(e) {
  const fechas = [];
  (e.intervenciones || []).forEach(i => { if (i.tipo === "CORRECTIVA") fechas.push(i.fecha); });
  (HISTORIAL[e.patente] || []).forEach(i => { if (i.tipo === "CORRECTIVA") fechas.push(i.fecha); });
  if (!fechas.length) return null;
  return fechas.sort().reverse()[0];
}

const equipos = JSON.parse(fs.readFileSync(
  path.join(__dirname, "equipos.json"),
  "utf-8"
));

const CAT_ORDEN = ["CAMION", "CARGADOR", "EXCAVADORA", "CAMIONETA", "BATEA", "CAMA_BAJA", "GENERADOR"];
const CAT_LABEL = { CAMION: "Camión", CARGADOR: "Cargador", EXCAVADORA: "Excavadora", CAMIONETA: "Camioneta", BATEA: "Batea", CAMA_BAJA: "Cama Baja", GENERADOR: "Generador" };
...
const resultado = equipos
  .map(e => {
    ...
    return {
      ...
      categoriaLabel: CAT_LABEL[e.categoria] || e.categoria,
      patente: e.patente || "—",
      marca: e.marca || "",
      modelo: e.modelo || "",
      m3: MAESTRO[e.patente]?.m3 || null,
      operador: e.operador || "Sin asignar",
      unidad: esCam ? "km" : "h",
      lecturaLabel: esCam ? "Odómetro" : "Horómetro",
      lecturaActual: e.horaActual || 0,
      lecturaFecha: e.horaActualFecha || null,
      odometro: esCamion ? (e.odometro || 0) : null,
      odometroFecha: esCamion ? (e.odometroFecha || null) : null,
      ultimaPreventivaFecha: e.ultimaPreventivaFecha || ultimaFechaHist(e.patente, "PREVENTIVA"),
      ultimaGeneralFecha: e.ultimaGeneralFecha || ultimaFechaHist(e.patente, "GENERAL"),
      ultimaCorrectivaFecha: ultimaFechaCorrectiva(e),
      salud: est.salud,
      estPrev: est.estPrev,
      estGen: est.estGen,
      restPrev: est.restPrev,
      restGen: est.restGen,
      restPrevKm: est.restPrevKm,
      restGenKm: est.restGenKm,
      useKmForPrev: est.useKmForPrev,
      useKmForGen: est.useKmForGen,
      notas: e.notas || "",
    };
  })
  .sort((a, b) => {
    const ia = CAT_ORDEN.indexOf(a.categoria), ib = CAT_ORDEN.indexOf(b.categoria);
    if (ia !== ib) return ia - ib;
    return a.patente.localeCompare(b.patente);
  });

fs.writeFileSync(
  path.join(__dirname, "reporte_data.json"),
  JSON.stringify(resultado, null, 2)
);
console.log(`Procesados ${resultado.length} equipos.`);
